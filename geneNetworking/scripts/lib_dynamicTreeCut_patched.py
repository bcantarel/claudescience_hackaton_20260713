...
def cutreeHybrid(link, distM,
    ...
    for merge in range(nMerge):
        if dendro_height[merge] <= cutHeight:
            ...
            if dendro_merge[merge, 0] < 0 and dendro_merge[merge, 1] < 0:
                ...
                clusts = np.atleast_1d(IndMergeToBranch[dendro_merge[merge,] - 1])
                sizes = branch_size[clusts - 1]
                # Note: for 2 elements, rank and order are the same.
                rnk = rankdata(sizes, method = "ordinal").astype(int)
    ...
    if UnlabeledExist:
        if len(Sizes) > 1:
            SizeRank = np.append(1, rankdata(-Sizes.iloc[1:len(Sizes)], method="ordinal")+1)
        else:
            SizeRank = np.array([1])
